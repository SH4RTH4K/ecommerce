@php($dmNameIsBengali = (bool)preg_match('/[\x{0980}-\x{09FF}]/u', $mode['site_name']))
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
...
<body>
    <main class="dm-shell">
        ...
        <div class="dm-content">
            @if($mode['logo'])<img class="dm-logo" src="{{ asset($mode['logo']) }}" alt="{{ $mode['site_name'] }}">@else<div class="dm-brand {{ $dmNameIsBengali ? 'is-bengali' : '' }}" style="--brand-name-font-size:{{ $brandNameFontSize }}px" @if($dmNameIsBengali) lang="bn" @endif>{{ $mode['site_name'] }}</div>@endif
