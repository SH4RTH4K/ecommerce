...
<div id="content" class="span10 au-page">
    ...
    <div class="au-box">
        ...
        <table class="au-commits"><thead><tr><th>Date</th><th>Branch</th><th>From</th><th>To</th><th>Comment</th><th>Status</th><th>Action</th></tr></thead><tbody>@forelse($history as $item)<tr><td>{{ optional($item->created_at)->format('Y-m-d H:i') }}</td><td>{{ $item->branch }}</td><td><code>{{ substr((string)$item->previous_commit,0,12) }}</code></td><td><code>{{ substr((string)($item->deployed_commit ?: $item->target_commit),0,12) }}</code></td><td>{{ $item->commit_subject ?: 'Commit message unavailable' }}</td><td>{{ strtoupper($item->status) }}</td><td>@if($item->status==='success' && !$item->rollback_of)<form method="post" action="{{ route('admin.application-update.rollback',$item->id) }}" onsubmit="return confirm('Rollback source to the recorded previous commit? Database migrations will not be reversed.')">@csrf<button class="au-btn gray">Rollback source</button></form>@endif</td></tr>@empty<tr><td colspan="7" class="au-muted">No deployments recorded.</td></tr>@endforelse</tbody></table>
