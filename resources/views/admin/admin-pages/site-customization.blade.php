...
@php
    ...
    $checks = [
        ['Business identity', (bool)$setting('site_name'), 'identity'],
        ['Customer contact', (bool)($setting('phone') || $setting('support_email') || $setting('whatsapp_number')), 'contact'],
        ['Store address', (bool)$setting('shop_address'), 'contact'],
        ['Search description', (bool)$setting('default_meta_description'), 'seo'],
        ['Header branding', (bool)$setting('site_name'), 'identity'],
    ];
    $complete = collect($checks)->where(1, true)->count();
    $percent = (int)round(($complete / count($checks)) * 100);
...
<div id="content" class="span10 ws-page">
    ...
    <form id="website-settings-form" method="post" enctype="multipart/form-data" action="{{ url('/site-settings') }}">{{ csrf_field() }}
        <div class="ws-layout">
            ...
            <main>
                            ...
                            <div class="ws-field">
                                ...
                                <div class="ws-upload {{ $errors->hasAny(['logo','remove_logo','logo_resize_width','logo_resize_height']) ? 'has-error' : '' }} {{ $removeLogoRequested ? 'is-removing' : '' }}">
                                    ...
                                    <div class="ws-resize-box">
                                        ...
                                        <small class="ws-resize-note">Allowed output: 120–2400 px wide and 40–1200 px high. The whole image is fitted inside the selected size without stretching or cropping.</small>
                ...
                <section class="ws-panel" data-settings-panel="contact">
                    <div class="ws-card"><div class="ws-card-head"><h2>Customer contact and location</h2><p>Make it easy for customers to reach and visit your business.</p></div><div class="ws-card-body">
                        ...
                        <div class="ws-contact-layout">
                            ...
                            <aside class="ws-contact-preview" aria-label="Customer contact preview" aria-live="polite"><small>Customer preview</small><h3 data-brand-output>{{ $setting('site_name', config('app.name')) }}</h3><p>This is how your essential contact information will read to customers.</p>
                ...
                <section class="ws-panel" data-settings-panel="content">
                    <div class="ws-card"><div class="ws-card-head"><h2>Storefront messages</h2><p>Short text used on the homepage and global notice area.</p></div><div class="ws-card-body"><div class="ws-grid">
                        ...
                        <div class="ws-field"><label for="hero_side_text">Homepage feature description</label><input id="hero_side_text" name="hero_side_text" maxlength="240" value="{{ $setting('hero_side_text') }}" data-count><small class="ws-help"><span class="ws-counter"></span></small></div>
                    </div></div></div>
                    <div class="ws-card"><div class="ws-card-head"><h2>Footer content</h2><p>Information repeated at the bottom of storefront pages.</p></div><div class="ws-card-body"><div class="ws-grid">
                        ...
                        <div class="ws-field full"><label for="copyright_text">Copyright line</label><input id="copyright_text" name="copyright_text" maxlength="255" value="{{ $setting('copyright_text', '© {year} '.config('app.name').'. All rights reserved.') }}"><small class="ws-help">Use <code>{year}</code> to keep the year current automatically.</small></div>
                    </div></div></div>
                ...
                <section class="ws-panel" data-settings-panel="seo">
                    <div class="ws-card"><div class="ws-card-head"><h2>Search engine defaults</h2><p>Fallback information for pages without their own SEO content.</p></div><div class="ws-card-body">
                        ...
                        <div class="ws-grid">
                            ...
                            <div class="ws-field full"><label>Social sharing image</label><div class="ws-upload">@if(!empty($settings['default_og_image']))<img class="ws-og" src="{{ asset($settings['default_og_image']) }}" alt="Current social sharing image">@endif<input type="file" name="seo_image" accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp" data-file-label><span class="ws-file-name"></span><small class="ws-help">Recommended 1200 × 630px, PNG/JPG/WebP, up to 4MB.</small></div></div>
                        </div>
                    </div></div>
                </section>
        ...
        <div class="ws-savebar"><span><i class="icon-info-sign"></i> Changes become public immediately after saving.</span><button type="submit" class="btn btn-primary"><i class="halflings-icon white ok"></i> Save website settings</button></div>
    </form>
</div>
<script>
document.addEventListener('DOMContentLoaded',function(){
    ...
    var requested=location.hash.replace('#','');if(document.querySelector('[data-settings-panel="'+requested+'"]'))openTab(requested);
    ...
    function setAssetRemoval(key,removing){
        ...
        if(button){button.setAttribute('aria-pressed',removing?'true':'false');var label=button.querySelector('span');if(label)label.textContent=removing?(key==='logo'?'Undo logo removal':'Undo icon removal'):(key==='logo'?'Remove current logo':'Remove current icon')}
    ...
    var brandInput=document.querySelector('[data-brand-preview]'),brandOutput=document.querySelector('[data-brand-output]');if(brandInput&&brandOutput)brandInput.addEventListener('input',function(){brandOutput.textContent=this.value.trim()||'Your business'});
