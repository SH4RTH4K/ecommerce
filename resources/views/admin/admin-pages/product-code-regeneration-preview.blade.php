...
<main id="content" class="span10 pcc">
    <div class="pcc-hero"><div><h1>Existing Code Impact Preview</h1><p>Dry-run only. No codes, sequences, or history are changed until you explicitly apply this preview.</p></div><div class="summary"><span class="pcc-pill">{{ strtoupper($configuration->code_type) }}</span><span class="pcc-pill">{{ $preview['total'] }} affected</span><span class="pcc-pill">{{ $preview['ready'] }} ready</span><span class="pcc-pill">{{ $preview['conflicts'] }} conflicts</span><span class="pcc-pill">{{ $preview['sequence_preserved'] ?? 0 }} sequence(s) preserved</span><span class="pcc-pill">{{ $preview['sequence_allocated'] ?? 0 }} sequence(s) allocated</span></div></div>
    @if(session('exception') || $errors->any())
        <div class="pcc-banner" style="background:#fff5f4;border-color:#edb0a8">
            <div><strong>Code regeneration was not applied</strong><small>{{ session('exception') ?: $errors->first() }}</small></div>
        </div>
    @endif
    ...
    <div class="pcc-card"><div class="pcc-head"><h2>Old and proposed codes</h2><p>Proposed codes use a fresh sequence for this format, starting at the configured starting number. Existing numeric endings are not reused.</p></div><div class="pcc-body">
        <form method="post" action="{{ route('product-code-configuration.regeneration.apply') }}" onsubmit="return confirm('Type REGENERATE to confirm this controlled code change. Historical transaction snapshots will not be rewritten.')">
            ...
            <div class="pcc-table-wrap"><table class="table table-bordered pcc-table" style="min-width:930px"><thead><tr><th>Select</th><th>Sl. No.</th><th>Name</th><th>Current code</th><th>Proposed code</th><th>Status</th></tr></thead><tbody>
            ...
            </tbody></table><script>(function(){document.querySelectorAll('.pcc-table tbody tr').forEach(function(row,index){if(row.cells.length===5){var serial=row.insertCell(1);serial.textContent=index+1;serial.className='pcc-serial';}});})();</script></div>
            <div class="pcc-grid-form" style="margin-top:18px"><div class="span-2"><label>Reason for code regeneration</label><textarea name="reason" required placeholder="Changed code format to include Company Code.">{{ old('reason') }}</textarea></div><div><label>Strong confirmation</label><input name="confirmation" required placeholder="Type REGENERATE" value="{{ old('confirmation') }}"></div></div>
