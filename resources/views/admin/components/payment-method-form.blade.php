...
@endphp
@if(!$m || !request()->routeIs('payment-methods.index'))
<style>
...
<form class="pm-form pm-adaptive-form" method="post" enctype="multipart/form-data" action="{{ route('payment-methods.store') }}">
 ...
 <section class="pm-wizard-step" data-step="2" hidden>
  ...
  <div class="pm-section"><h3>How will this method process payment?</h3><div class="pm-grid">
   <div class="pm-field pm-conditional" data-methods="mobile_financial_service payment_gateway card_payment qr_payment manual_payment custom" data-always-submit><label>Integration mode *</label><select name="integration_mode">@foreach(['offline'=>'Offline collection','manual'=>'Manual customer payment','hosted_checkout'=>'Hosted checkout page','redirect'=>'Provider redirect','api'=>'Direct API'] as $v=>$l)<option value="{{ $v }}" {{ $pv('integration_mode','offline')===$v?'selected':'' }}>{{ $l }}</option>@endforeach</select></div>
...
(function(form){
 ...
 function showStep(number,shouldScroll){step=Math.max(1,Math.min(4,number));form.querySelectorAll('[data-step]').forEach(function(panel){panel.hidden=Number(panel.dataset.step)!==step});form.querySelectorAll('[data-go-step]').forEach(function(button){button.classList.toggle('is-active',Number(button.dataset.goStep)===step)});form.querySelector('[data-previous-step]').hidden=step===1;form.querySelector('[data-next-step]').hidden=step===4;form.querySelector('[data-save-method]').hidden=step!==4;if(step===4){var label=form.querySelector('[name="customer_label"]').value||form.querySelector('[name="name"]').value||'This method';var env=form.querySelector('[name="environment"]');form.querySelector('[data-review-summary]').textContent=label+' will be saved as '+(form.querySelector('[name="is_active"]:checked')?'Enabled':'Disabled')+(env&&!env.disabled?' in '+env.value.toUpperCase()+' mode':'')+'.';}if(shouldScroll)form.scrollIntoView({behavior:'smooth',block:'start'})}
 ...
 form.querySelectorAll('[data-go-step]').forEach(function(button){button.addEventListener('click',function(){showStep(Number(button.dataset.goStep),true)})});
 form.querySelector('[data-next-step]').addEventListener('click',function(){if(!currentStepIsValid())return;showStep(step+1,true)});
 form.querySelector('[data-previous-step]').addEventListener('click',function(){showStep(step-1,true)});
 refreshConditional();showStep(1,false);
