...
<section class="dt-card" data-transfer>
    <header class="dt-head">
        <div class="dt-title">
            ...
            <div>
                <h3>{{ $transferLabel }} Import &amp; Export</h3>
                <p>CSV templates, transactional validation, and safe create/update matching.</p>
            </div>
        </div>
        ...
        <form method="post" enctype="multipart/form-data" action="{{ route('admin-data.import',$resource) }}">{{ csrf_field() }}
            <div class="dt-grid">
                <label class="dt-drop">
                    ...
                    <span><strong><i class="icon-cloud-upload"></i> Drop CSV here or browse</strong><small data-file-name>Maximum 5 MB · UTF-8 CSV recommended</small></span>
                </label>
                <div class="dt-field">
                    <label>Import behavior</label>
                    <select name="mode">
                        <option value="upsert">Create new and update matches</option>
                        <option value="create">Create new; skip matches</option>
                    </select>
                </div>
