...
<aside id="sidebar-left" class="span2" aria-label="Administration navigation">
    <div class="sidebar-nav nav-collapse">
        ...
        <nav class="admin-menu">
            ...
            <section class="admin-menu-group {{ $groupOpen(['add-product','manage-product','edit-product/*','pc-builder-settings','manage-category','add-category','edit-category/*','manage-subCategory','add-subCategory','edit-subCategory/*','manage-manufacturer','add-manufacturer','edit-manufacturer/*','catalog-hierarchy*','catalog-imports*','catalog-attributes*']) }}" data-menu-group="catalog">
                ...
                <div class="admin-menu-items">
                    ...
                    <a class="admin-menu-link {{ $active(['catalog-hierarchy*','manage-manufacturer','manage-manufacturers','add-manufacturer','edit-manufacturer/*']) }}" href="{{ url('/catalog-hierarchy') }}"><i class="icon-certificate"></i><span>Step 3 · Companies, Brands &amp; Product Lines (optional)</span></a>
                    <a class="admin-menu-link admin-menu-sub {{ $active(['manage-manufacturer','manage-manufacturers']) }}" href="{{ url('/manage-manufacturer') }}"><i class="icon-star"></i><span>Featured Brands</span></a>
                    ...
                    <a class="admin-menu-link {{ $active('pc-builder-settings') }}" href="{{ url('/pc-builder-settings') }}"><i class="icon-wrench"></i><span>PC Builder Rules</span></a>
            ...
            <section class="admin-menu-group {{ $groupOpen(['admin-notifications*','product-code-configuration*','system-health*','system-monitor*','integrations*','recycle-bin*','orphan-media*']) }}" data-menu-group="system">
                ...
                <div class="admin-menu-items">
                    ...
                    @if(in_array('settings',(array)session('admin_permissions',[])))<a class="admin-menu-link {{ $active('admin/system/*') }}" href="{{ route('admin.application-update') }}"><i class="icon-refresh"></i><span>Application Update</span></a>@endif
