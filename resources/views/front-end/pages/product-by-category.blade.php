...
<section class="lt-section lt-listing-page" style="padding-top:0">
    ...
    <div class="lt-page-hero">
        <div>
            ...
            <p>Browse the latest products in this category, then narrow the results by brand, price, or attribute filters.</p>
        </div>
        <div class="lt-page-stats">
            ...
            <div class="lt-page-stat">
                <strong>{{ $manufacturers->count() }}</strong>
                <span>Brands</span>
            </div>
            <div class="lt-page-stat">
                <strong>{{ $attributeFilters->count() }}</strong>
                <span>Filters</span>
            </div>
        </div>
    </div>
    ...
    <div class="lt-catalog-layout">
        <aside class="lt-filter-panel">
            <form method="get">
                <div class="lt-filter-title">
                    <h2>Filter products</h2>
                    <a href="{{ url('/product-by-category/'.$category->category_id) }}">Reset</a>
                </div>

                <label>Search in category
                    <input type="search" name="q" value="{{ request('q') }}" placeholder="Product or model">
                </label>
                ...
                <label>Brand
                    <select name="manufacturer">
                        ...
                        @foreach($manufacturers as $brand)
                            <option value="{{ $brand->manufacturer_id }}" {{ (string) request('manufacturer') === (string) $brand->manufacturer_id ? 'selected' : '' }}>
                    ...
                    <label>{{ $attribute->name }}
                        <select name="attributes[{{ $attribute->id }}]">
                            ...
                            @foreach($attribute->values as $value)
                                <option value="{{ $value }}" {{ request('attributes.'.$attribute->id) === $value ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                ...
                <label class="lt-check">
                    <input type="checkbox" name="availability" value="in-stock" {{ request('availability') === 'in-stock' ? 'checked' : '' }}>
                    In stock only
                </label>
        ...
        <div class="lt-catalog-results">
            <form class="lt-sortbar" method="get">
                @foreach(request()->except(['sort','per_page','page']) as $name => $value)
                    @if(is_array($value))
                        @foreach($value as $nestedKey => $nestedValue)
                            <input type="hidden" name="{{ $name }}[{{ $nestedKey }}]" value="{{ $nestedValue }}">
                        @endforeach
                    @else
                ...
                <label>Show
                    <select name="per_page" onchange="this.form.submit()">
                        @foreach([12, 24, 48] as $size)
                            <option value="{{ $size }}" {{ $products->perPage() === $size ? 'selected' : '' }}>{{ $size }}</option>
                        @endforeach
                    </select>
                </label>
            </form>
            ...
            <div class="lt-product-grid">
                ...
                @empty
                    <div class="lt-empty">No products match these filters.</div>
                @endforelse
            </div>
