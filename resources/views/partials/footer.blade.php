@php
    $siteName = $brandName;
    $siteLogo = isset($siteSettings['site_logo']) && $siteSettings['site_logo'] ? $siteSettings['site_logo'] : 'asset/front-end/img/ecommerce-logo.png';
...
<footer class="lt-footer">
    <div class="lt-container lt-footer-grid">
        <div><img class="lt-footer-logo" src="{{ asset($siteLogo) }}" alt="{{ $siteName }}" style="filter:none"><p>{{ isset($siteSettings['footer_description']) && $siteSettings['footer_description'] ? $siteSettings['footer_description'] : 'Your trusted destination for computers, networking equipment, accessories, and dependable after-sales service.' }}</p>@if(isset($siteSettings['site_tagline']) && $siteSettings['site_tagline'])<p><strong>{{ $siteSettings['site_tagline'] }}</strong></p>@endif</div>
