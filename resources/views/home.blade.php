...
<body>
    ...
    <main id="main-content">
        ...
        <section class="lt-hero lt-container" aria-label="Featured promotions">
            ...
            <aside class="lt-hero-side hfc-layout-{{ strtolower($featureCardConfig['layout']) }} {{ $featureCards->count() === 1 ? 'hfc-single-card' : '' }} {{ !empty($featureCardConfig['equal_height']) ? 'hfc-equal-height' : '' }}" style="--hero-card-gap:{{ (int) $featureCardConfig['card_gap'] }}px" data-feature-cards data-layout="{{ $featureCardConfig['layout'] }}" data-interval="{{ (int) $featureCardConfig['slider_interval'] }}" data-autoplay="{{ $featureCardConfig['slider_autoplay'] ? '1' : '0' }}" data-pause="{{ $featureCardConfig['pause_on_hover'] ? '1' : '0' }}">
                @foreach($featureCards as $card)
                    @php $cardStyle = strtolower((string) $card->color_style); $cardLink = $card->resolvedLink(); $cardImage = $card->resolvedImage(); $cardClasses = 'hfc-store-card type-'.strtolower((string) $card->card_type).' is-'.$cardStyle.' text-'.$card->text_position.' fit-'.$card->image_fit.' pos-'.$card->image_position.' overlay-'.$card->overlay_style; $cardInline = ''; foreach (['background'=>'custom_background_color','color'=>'custom_text_color'] as $css => $key) { if (preg_match('/^#[0-9a-fA-F]{6}$/', (string) $card->{$key})) $cardInline .= '--hfc-'.$css.':'.$card->{$key}.';'; } @endphp
                    <div class="{{ $cardClasses }}" @if($cardInline) style="{{ $cardInline }}" @endif data-feature-card data-clickable-area="{{ $card->clickable_area }}" data-card-link="{{ $cardLink ?? '' }}" data-card-new-tab="{{ $card->open_in_new_tab ? '1' : '0' }}">
                        @if($cardImage)
                            @if($cardLink && $card->clickable_area === 'IMAGE_ONLY')<a class="hfc-image-link" href="{{ $cardLink }}" @if($card->open_in_new_tab) target="_blank" rel="noopener" @endif aria-label="{{ $card->title ?: $card->name }}">@endif
                            <img src="{{ asset($cardImage) }}" alt="{{ $card->resolvedAlt() }}" loading="{{ $loop->index < 2 ? 'eager' : 'lazy' }}">
                            @if($cardLink && $card->clickable_area === 'IMAGE_ONLY')</a>@endif
                        @endif
                        ...
                        @if($cardLink)<span class="hfc-click-indicator" aria-hidden="true" title="Click to open"><i class="fa fa-hand-pointer-o"></i></span>@endif
        ...
        <section class="lt-section lt-brands-section"><div class="lt-container"><div class="lt-section-heading"><div><span>Shop trusted names</span><h2>Popular Brands</h2></div></div><div class="lt-brand-grid">@forelse($brands as $brand) @php $brandLabel = trim($brand->manufacturer_name); $brandLength = mb_strlen($brandLabel); $brandFontSize = $brandLength > 12 ? 13 : ($brandLength > 9 ? 15 : 18); $brandIcon = $featuredBrandIcons[(string) $brand->manufacturer_id] ?? null; $brandImage = $featuredBrandImages[(string) $brand->manufacturer_id] ?? null; @endphp<a href="{{ url('/all-manufacturer-by-id/'.$brand->manufacturer_id) }}" title="{{ $brandLabel }}"><span class="lt-brand-initials">@if($brandImage)<img src="{{ asset($brandImage) }}" alt="" loading="lazy">@elseif($brandIcon)<i class="fa {{ $brandIcon }}" aria-hidden="true"></i>@else{{ strtoupper(substr($brandLabel, 0, 2)) }}@endif</span><strong class="lt-brand-name" style="font-size:{{ $brandFontSize }}px">{{ $brandLabel }}</strong></a>@empty<div class="lt-empty">Select published brands in the admin panel to populate this section.</div>@endforelse</div></div></section>
    </main>
